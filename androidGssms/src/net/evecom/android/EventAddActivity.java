...
public class EventAddActivity extends Activity {
    ...
    private void init() {
        ...
        SharedPreferences sp = EventAddActivity.this.getSharedPreferences("T_EVENT", MODE_PRIVATE);
        Editor editor = sp.edit();
        editor.putString("EVENTINITGRID", 
                ShareUtil.getString(getApplicationContext(),
                        "SESSION", "AREAID", "0")); // 默认是连江县
        editor.commit();
    }
    ...
    private void initView() {
        ...
        if (ShareUtil.getString(getApplicationContext(), "SESSION", "AREAID", "1").equals(
                ShareUtil.getString(getApplicationContext(), "SESSION", "ISZNBM", "-1"))) {
            fswlTextView.setText("");
        } else if (ShareUtil.getString(getApplicationContext(), "SESSION", "AREANAME", "").equals("连江县")) {
            fswlTextView.setText("");
        } else {
    ...
    public void findpicture_onclick(View v) {
        ...
        Dialog schDia = new AlertDialog.Builder(EventAddActivity.this).setIcon(R.drawable.login_error_icon)
                .setTitle("请选择").setItems(filetypes, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dia, int which) {
                        // if(null!=fileListg&&fileListg.size()>3){
                        // toast("上传文件不要超过5个");
                        // return ;
                        // }
        ...
        Dialog schDia = new AlertDialog.Builder(EventAddActivity.this).setIcon(R.drawable.login_error_icon)
                .setTitle("请选择").setItems(sflx, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dia, final int which_p) {
                        ...
                        Dialog mchDia = new AlertDialog.Builder(EventAddActivity.this)
    ...
    public void fswl(View v) {
        System.out.println(ShareUtil.getString(getApplicationContext(), "SESSION", "AREALEV", ""));
        String AREALEV = ShareUtil.getString(getApplicationContext(), "SESSION", "AREALEV", "");
        String AREAID = ShareUtil.getString(getApplicationContext(), "SESSION", "AREAID", "");
        if (null != AREALEV && "4".equals(AREALEV)) {
            getTreeTwo(AREAID);
        } else if (null != AREALEV && "5".equals(AREALEV)) {
            getTreeThree(AREAID);
        } else if (null != AREALEV && "6".equals(AREALEV) && fswlTextView.getText().toString().trim().length() > 0) {
            toast("所属网格为" + ShareUtil.getString(getApplicationContext(), "SESSION", "AREANAME", "") + ",无需再选");
        } else {
            getTreeOne("");
        }
        // getTreeOne("");
    ...
    private void submit() {
        ...
        if (sjmcEditText.getText().toString().trim().length() < 1) {
            DialogToast("请填写事件名称！");
            return;
        } else if ("连江县".equals(fswlTextView.getText().toString().trim())) {
            DialogToast("请选择发生网格！");
            return;
        } else if ("".equals(fswlTextView.getText().toString().trim())) {
            DialogToast("请选择发生网格！");
            return;
        }
        ...
        final AlertDialog.Builder builder = new AlertDialog.Builder(EventAddActivity.this);
        ...
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                progressDialog = ProgressDialog.show(EventAddActivity.this, "提示", "正在保存，请稍等...");
                formSubmit();
                // fileSubmitAfterForm();
            }
        });
     ...
     * 上传文件
     */
    private void fileSubmitAfterForms(String table_id) {
        // msg_listData2 = new Message();
        if (fileListg.size() == 0) {
            if (null != progressDialog) {
                progressDialog.dismiss(); // 关闭进度条
        ...
        FinalHttp fh = new FinalHttp();
        String furl = HttpUtil.BASE_URL + "teventAndroid/uploadPics";
        fh.post(furl, params, new AjaxCallBack<String>() {
            @Override
            public void onLoading(long count, long current) {
                System.out.println(current + "/" + count);
    ...
    public void eventfh(View v) {
        AlertDialog.Builder builder1 = new AlertDialog.Builder(EventAddActivity.this);
        builder1.setTitle("提示信息");
        builder1.setIcon(R.drawable.qq_dialog_default_icon);// 图标
        builder1.setMessage("退出数据将丢失,是否要退出登记？");
    ...
    private void getTreeOne(final String sbd_id) {
        progressDialog_getInfo = ProgressDialog.show(EventAddActivity.this, "提示", "正在获取数据，请稍等...");
        list.removeAll(list);
        new Thread(new Runnable() {
            @Override
            public void run() {
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case MESSAGETYPE_01:
                    ...
                    View messageView = LayoutInflater.from(EventAddActivity.this).inflate(R.layout.base_dialog_info,
                            null);
                    // 分页设置
                    final WheelView country = (WheelView) messageView.findViewById(R.id.country);
                    country.setVisibleItems(3);
                    country.setViewAdapter(new CountryAdapter(EventAddActivity.this, list));
                    ...
                    baseDict = list.get(0);
                    Sysindex = 0;
                    Dialog delDia = new AlertDialog.Builder(EventAddActivity.this)
                            .setIcon(R.drawable.qq_dialog_default_icon).setTitle("请选择城镇！").setView(messageView)
                            .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dia, int which) {
                                    SharedPreferences sp = EventAddActivity.this.getSharedPreferences("T_EVENT",
                                            MODE_PRIVATE);
                                    Editor editor = sp.edit();
